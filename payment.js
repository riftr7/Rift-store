    ...
    if (currentOrder.paymentMethod === 'Zain Cash') {
      paymentDetails.innerHTML = `
        <div style="background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); border-radius: 12px; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 0; font-size: 18px; font-weight: bold;">${isArabic ? 'رقم الهاتف' : 'Phone Number'}: <span id="zain-number">+9647758918504</span></p>
      ...
      document.getElementById('copy-zain-number').addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText('+9647758918504');
      ...
      paymentDetails.innerHTML = `
        <div style="background: rgba(243,186,47,0.1); border: 1px solid rgba(243,186,47,0.3); border-radius: 12px; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 0; font-size: 16px; word-break: break-all;">${isArabic ? 'معرف Binance' : 'Binance ID'}: <span id="binance-id" style="font-weight:bold; color: #f3ba2f;">1259953523</span></p>
      ...
      document.getElementById('copy-binance-id').addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText('1259953523');
      ...
      paymentDetails.innerHTML = `
        <div style="background: rgba(0,188,212,0.1); border: 1px solid rgba(0,188,212,0.3); border-radius: 12px; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 0; font-size: 18px; font-weight: bold;">${isArabic ? 'رقم الهاتف' : 'Phone Number'}: <span id="superqi-number">+9647758918504</span></p>
      ...
      paymentDetails.innerHTML = `
        <div style="background: rgba(235,0,27,0.1); border: 1px solid rgba(235,0,27,0.3); border-radius: 12px; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 0; font-size: 16px; word-break: break-all;">${isArabic ? 'معرف Mastercard' : 'Mastercard ID'}: <span id="mastercard-id" style="font-weight:bold; color: #eb001b;">910125497651</span></p>
      ...
      document.getElementById('copy-mastercard-id').addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText('910125497651');
